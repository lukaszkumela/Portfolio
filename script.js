...
const shouldVidLoad = () => {
    if(window.innerWidth > 767) {
        source.setAttribute("src", "video/Neon-4.mp4");
        vid.load();
        vid.playbackRate = 2;
...
let headerNumber = 0;
const headersArray = ["Łukasz Kumela", "design", "code", "create"];
const colors = ["#fff", "#0f2", "#4666FF", "#FF8811"]
...
vid.addEventListener("ended", () => {
    if (vid.style.display != "none") {
        ...
        if(played2times) {
            ...
            headerNumber === 0 ? mainSubheader.innerText = "web developer" : mainSubheader.innerText = "";
            vid.style.filter = `hue-rotate(${Math.floor(Math.random() * 361)}deg)`;
            if(vid.style.display !="none") {
                mainHeader.innerText = headersArray[headerNumber];
                headersArray[headerNumber] != "Łukasz Kumela" ? mainHeader.style.top = "30%":mainHeader.style.top = "28%"
...
vidSwitch.addEventListener("click", ()=> {
    ...
    if(vidOff) {
        ...
        vid.pause();
        mainHeader.innerText = "Łukasz Kumela";
        mainSubheader.innerText = "web developer"
